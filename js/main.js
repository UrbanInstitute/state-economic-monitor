...
function buildBarChart(chartData, indicator, unit, states, endDate, active){
...
buildMap(data, key, colorScale)


}
function buildMap(data, key, colorScale){
	var width = 621,
		height = .6 * width
...
d3.json("data/states.json").then(function(us) {
	...
	d3.select("#topojsonData").data([us])
	// console.log(data)
...
for(var i = 1; i < tickBreaks.length; i++){
...
// d3.select("#lineAxisBlocker").node().parentNode.appendChild(d3.select("#lineAxisBlocker").node())

// d3.select("#linesContainer").append("rect")
// 	.style("fill", "#fff")
// 	.attr("x", -margin.left)
// 	.attr("width", margin.left)
// 	.attr("y", 0)
...
function init(allData){
	...
	buildBarChart(allData.data, indicator, unit, states, endDate, activeBars)
